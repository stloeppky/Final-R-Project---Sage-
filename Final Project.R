...
plot(AA.Cultus$year, AA.Cultus$datavalue, 
     ...
     col = "red",
     xlim = c(1950, 1994),
     abline(lm(AA.Cultus$year ~ AA.Cultus$datavalue), col = "blue"))
...
ggplot(data = AA.Cultus, aes(x = year, y = datavalue)) +
  geom_point() +
...
# trying to make the columns that I want? 
...
# write to CSV file 
write.csv(mean.annual.precip, paste(p.fp.csv, "f.fp.csv"), row.names = FALSE)
...
# Trying to make a plot in GG plot 
ggplot(data = mean.annual.precip, aes(x = Group.1, y = x)) +
  geom_point() +
...
average_abundace <- AA.Cultus$datavalue
...
ggplot(merged.data, aes(x = year, y = c(log(mean_yearly_precip), log(datavalue)), type = 'l'))
...
merged.data.plot <- ggplot(merged.data, aes(x = year)) + 
  ...
  geom_line(aes(y = log(log(datavalue)), color = 'average abundace'))


ggplot(merged.data, aes(x = year, y = datavalue, size = mean_yearly_precip)) +
...
# one way ANOVA 
one.way.anova <- aov(datavalue ~ mean_yearly_precip, data = merged.data)
